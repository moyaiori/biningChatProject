...
public class RoomManager {
	...
	public void removeRoom(String roomName) {
		roomName = room.getName(); // getName메소드를 이용하여 방 제목을 가져옴.
		for (int i = 0; i < ROOM_NUMBER; i++) {
			if (roomList.keys().equals(roomName)) { // 만약 roomList의 key값과 매개변수로 받은
												// name의 이름이 같으면
				roomList.remove(roomName); // 해당 방이름을 가진 방을 삭제함
			}
		}
		totalRoomCount--;
	}
