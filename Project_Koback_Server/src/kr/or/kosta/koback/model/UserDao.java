...
public class UserDao {
   ...
   private static final int USER_NICKNAME_LENGTH =20;      //20바이트(최대 10글자)로 닉네임 저장
   ...
   private static final int USER_PHONE_LENGTH = 26;      //26바이트(00000000000 형식) 10 ~ 11자리 번호 저장
   ...
   /** ID검색해서 Password 가져오기 */
   public String idSearchPasswd(String getId) throws IOException {
      String id = "";
      String pass = "";
      for (int i = 0; i < userCount; i++) {
         randomAccessFile.seek((i * RECORD_LENGTH) + RECORD_COUNT_LENGTH);
         id = read(USER_ID_LENGTH);
         if (id.equals(getId)) {
            randomAccessFile.seek((i * RECORD_LENGTH) + RECORD_PASS_LENGTH);
            pass = read(USER_PASSWORD_LENGTH);
         }
         continue;
      }
      return pass;
