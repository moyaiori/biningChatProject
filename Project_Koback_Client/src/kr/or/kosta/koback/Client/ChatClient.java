...
public class ChatClient {
	...
	public void connect() throws IOException{
		...
		out = new DataOutputStream(socket.getOutputStream());	//예외처리는 throws로 서버에서 관리 하도록한다.
...
//										System.out.println("[서버 답장 채팅방 개설 201] "+responeMessage);
	...
	//										ui.getWaitRoomPanel().roomResult(MessageType.S_OPEN_RESULT,responeMessage);
										...
										break;
								/** 입장 결과 쪽 211 , 213 할것
								 * 1) WaitingChatRoomPanel 클래스를 접근하여 chatRoomOpenS()메소드 호출
								 * 2) chatRoomOpenS()에서 
								 * */		
...
//										ui.getWaitRoomPanel().chtRoomOpenS(결과응답값); //결과 응답이 true이면 채팅화면을 열기위해 이메소드를 호출!!!
										break;	
									/*[213] 비밀방 입장 결과 응답 */	
									case MessageType.S_SECRET_ENTRY_RESULT:	
//										System.out.printl n("[[213] 비밀방 입장 결과 응답] "+responeMessage);
//										sys
										break;		
									/*	[101]  로그인결과응답  */	
									case MessageType.S_LOGIN_RESULT:
								
										String connectionId = tokens[2];					//로그인 요청 한 클라이언트의 닉네임								
										...
										ui.getWaitRoomPanel().waitingList(waitingList);		//대기방 리스트에 출력하기위해 호출.
										
										if(ui.getUserLoginPanel().userId.equals(connectionId)){	//자기 자신 이외의 클라이언트가 접속 할경우
											if(!chatRoomLists.equals(null)){
//												String roomLists = "첫번째방,jo930408,1234,1,5,다들어와,비닝이,null,2,10,놀쟈,비닝이,null,2,10"; //tokens[4]
	...
	public void fileDownload() throws IOException{
		File file = new File(dao.getFilePath(), dao.getFile().getName());	//파일 객체만들기.
